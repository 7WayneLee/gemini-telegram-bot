...
async def _migrate_to_v6(connection: aiosqlite.Connection) -> None:
    await connection.execute("BEGIN IMMEDIATE")
    try:
        if not await _column_exists(
            connection,
            ...
            await connection.execute(
                "ALTER TABLE chat_sessions "
                f"ADD COLUMN {CHAT_SESSION_LANGUAGE_COLUMN} TEXT"
            )
...
async def _migrate_to_v8(connection: aiosqlite.Connection) -> None:
    await _run_schema_migration(
        connection,
        GROUP_THREAD_SCHEMA_SQL,
        version=8,
