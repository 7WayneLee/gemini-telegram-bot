...
- **預設白名單** —— 沒設定的 bot 誰都不理
- **群組模式** —— 可以把 bot 加進群組，以聊天為單位授權而非以使用者為單位，
  群組內使用固定的設定
...
| 變數 | 預設 | 說明 |
|---|---|---|
| `ALLOWED_CHAT_IDS` | 空 | 逗號分隔的群組 chat id（負數）。留空代表不核准任何群組 |
| `GROUP_MODEL` | `flash` | 群組固定使用的模型，由上游動態解析取得，並非硬編碼 |
| `GROUP_THREAD_RETENTION_DAYS` | `30` | 群組對話串記錄的保留天數 |
| `GROUP_THREAD_MAX_PER_CHAT` | `200` | 每個群組保留的對話串上限 |
...
| 指令 | 行為 |
|---|---|
| `/start`、`/help` | 顯示可用指令。`/help` 不在指令選單中，但仍可使用 |
| `/gemini <問題>` | 以這個問題開始一段新對話。作為回覆送出時，被引用訊息的內容會一併成為脈絡 |
| `/new` | 結束目前對話，開始新的一輪 |
...
| `/allow_chat <chat_id>` | 核准一個群組，即時生效。僅限私訊 |
| `/deny_chat <chat_id>` | 撤銷一個群組的核准，即時生效。僅限私訊 |
...
可以把 bot 加進群組。群組的行為與私訊不同，這些差異是刻意設計的。

### 私訊與群組的差別

| | 私訊 | 群組 |
|---|---|---|
| 對話 | 一段連續的對話。直接打字即可延續，`/new` 重置 | 一次一問。`/gemini <問題>` 每次都開始一段新對話；**回覆 bot 的訊息**才能延續該段對話。純文字不會送到 bot |
| 設定 | 模型、gem、暫時模式、延伸思考、介面語言都可以調整 | 固定且不可更改：模型為 `GROUP_MODEL`、介面為英文、延伸思考關閉、不套用 gem |
| 可用指令 | 全部 | `/gemini`、`/img`、`/help` |

### 為什麼群組需要指令或回覆

Telegram 的隱私模式預設開啟，因此 bot 在群組中只會收到指向自己的訊息：
明確指向該 bot 的指令，以及對該 bot 訊息的回覆。純文字與單純的 @提及收不到。

本專案刻意維持隱私模式開啟。關閉它等於讓 bot 收到群組中的每一則訊息，
而這個 bot 持有的是一個完整的 Google 登入 session。

群組中建議使用 `/gemini@你的bot名稱` 這種形式。Telegram 只保證明確指向某個 bot
的指令必定送達；不帶 `@` 的指令，只有在該 bot 是群組中最後發言的 bot 時才有保證。

### 群組授權

群組以聊天為單位授權，與個人白名單分開：

- 已核准的群組，成員不需要在 `ALLOWED_USER_IDS` 內即可使用
- 未核准的群組，bot 完全不回覆，並私訊管理員附上該群組的 chat id
- 管理員在私訊以 `/allow_chat <id>` 核准、以 `/deny_chat <id>` 撤銷，皆立即生效
- **核准一個群組，等於把使用權交給任何能被加進該群組的人。**
  群組成員由該群組自己的管理員控制，不由這個 bot 控制

### 管理指令只在私訊有效

`/setcookie` 與其他管理指令在群組中一律無效。在群組執行時，回覆的訊息與非管理員
收到的完全相同，因此沒有人能藉此推斷誰是管理員。
